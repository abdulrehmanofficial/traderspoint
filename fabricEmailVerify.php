  ...
  else
  {
	      $code=$_POST['code'];
           ...
           $verifyEmail=$_SESSION['verifyEmail'];
            ...
            $preQuery="select vCode from registerforFabric WHERE email='$verifyEmail'";
            $preData=mysqli_query($connect,$preQuery);
            $preRow=mysqli_fetch_array($preData);

            if($code==$preRow['vCode'])
            {
                $query="Update registerforFabric SET vStatus='Yes' WHERE email='$verifyEmail'";
	          ...
	          echo '<script>window.location="fabricEmailVerify.php#top"</script>';
              }
            }
            else
            {
