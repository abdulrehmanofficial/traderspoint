...
if(isset($_POST['registerOrders']))
{
  ...
  else
  {
   ...
   echo '<script>alert("Email Address is not a valid address !!")</script>';
	  ...
	  {
		echo '<script>alert("Company Cannot Contain Numbers !!")</script>';
	  }
	  ...
	  if($_POST['Pass']==$_POST['confirmPass'])
	  {
		  ...
		  $pass=md5($_POST['Pass']);
			  ...
			  $emailCheck="select * from registerfororders WHERE email='$email'";
			  $emailResult=mysqli_query($connect,$emailCheck);
			  
			  $CompanyCheck="select * from registerfororders WHERE company='$company'";
			  ...
			  if(mysqli_num_rows($emailResult)>0)
			  {
				  ...
				  echo '<script>window.location="registerForOrders.php"</script>';
			  }
			  ...
			  $query2="INSERT INTO registerfororders(name,company,designation,email,pass,vCode) VALUES('$name','$company',
			  ...
			  if ($connect->query($query2) === TRUE) 
			  {
	           ...
	           echo '<script>window.location="emailVerify.php#top"</script>';
              }
			  else 
			  {
...
<h3 style="font-family: 'Passion One', cursive;" class="text-center alert alert-info">Register for Orders</h3>
<br />
...
<center><input class="btn btn-success" type="submit" name="registerOrders" value="Register" /></center>
<br>
</form>
