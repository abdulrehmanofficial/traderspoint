   ...
   echo '<script>alert("Email Address is not a valid address !!")</script>';
	  ...
	  if($_POST['Pass']==$_POST['confirmPass'])
	  {
		  ...
		  $pass=md5($_POST['Pass']);
			  ...
			  $emailCheck="select * from registerforunits WHERE email='$email'";
			  $emailResult=mysqli_query($connect,$emailCheck);
			  
			  $CompanyCheck="select * from registerforunits WHERE company='$company'";
			  ...
			  if(mysqli_num_rows($emailResult)>0)
			  {
				  ...
				  echo '<script>window.location="registerforUnits.php"</script>';
			  }
			  ...
			  $query2="INSERT INTO registerforunits(name,company,designation,email,pass,vCode) VALUES('$name','$company',
			  ...
			  if ($connect->query($query2) === TRUE) 
			  {
	           ...
	           echo '<script>window.location="unitEmailVerify.php"</script>';
              }
			  else 
			  {
...
<h3 style="font-family: 'Passion One', cursive;" class="text-center alert alert-info">Register for Working Units</h3>
<br />
...
<center><input class="btn btn-success" type="submit" name="registerUnits" value="Register" /></center>
<br>
</form>
